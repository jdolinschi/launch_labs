<template>
  <div class="flex">
          ...
          <div class="px-2">
            <div class="py-4">
              <sidebar-item
                  ...
                  label="General"
                  :icon="generalIcon"
                  :is-active="activePanel === 'general'"
                  @open-panel="openPanel"
                  @close-panel="closePanel"
                  draggable="true"
                  @dragstart="dragStart('general')"
              />
            </div>
            <ul class="space-y-1 border-t border-gray-100 pt-4">
              ...
              <li>
                <sidebar-item
                    ...
                    label="Billing"
                    :icon="billingIcon"
                    :is-active="activePanel === 'billing'"
                    @open-panel="openPanel"
                    @close-panel="closePanel"
                    draggable="true"
                    @dragstart="dragStart('billing')"
                />
              </li>
              <li>
                <sidebar-item
                    ...
                    label="Invoices"
                    :icon="invoicesIcon"
                    :is-active="activePanel === 'invoices'"
                    @open-panel="openPanel"
                    @close-panel="closePanel"
                    draggable="true"
                    @dragstart="dragStart('invoices')"
                />
              </li>
              <li>
                <sidebar-item
                    ...
                    label="Account"
                    :icon="accountIcon"
                    :is-active="activePanel === 'account'"
                    @open-panel="openPanel"
                    @close-panel="closePanel"
                    draggable="true"
                    @dragstart="dragStart('account')"
                />
              </li>
            </ul>
          </div>
      ...
      <div class="sticky inset-x-0 bottom-0 border-t border-gray-100 bg-white p-2">
        <form action="#">
          <button type="submit" class="group relative flex w-full justify-center rounded-lg px-2 py-1.5 text-sm text-gray-500 hover:bg-gray-50 hover:text-gray-700">
            ...
            <span class="invisible absolute start-full top-1/2 ms-4 -translate-y-1/2 rounded-sm bg-gray-900 px-2 py-1.5 text-xs font-medium text-white group-hover:visible">Logout</span>
          </button>
        </form>
      </div>
    </div>
    ...
    <sidebar-panel :is-active="activePanel === 'invoices'" @close-panel="closePanel" @keep-open="keepOpen">
    ...
    <sidebar-panel :is-active="activePanel === 'account'" @close-panel="closePanel" @keep-open="keepOpen">
      <li>
        <details class="group [&_summary::-webkit-details-marker]:hidden">
          ...
          <ul class="mt-2 space-y-1 px-4">
            <li><a href="#" class="block rounded-lg px-4 py-2 text-sm font-medium text-gray-500 hover:bg-gray-100 hover:text-gray-700">Details</a></li>
    ...
    <canvas-component @drop="drop" @dragover.prevent />
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    const { activePanel, openPanel, closePanel, keepOpen } = usePanel()
    ...
    const dragStart = (content: string) => (event: DragEvent) => {
      event.preventDefault() // Prevent the browser from setting the URL as drag data
